...
| 执行参数 | model / temperature 不进 agent.md;模型归 `models.json` + `settings.json`。轮次策略在 `runtime.py`:`max_turns = 0` = 不限(交互式);headless 由 cli 传 `HEADLESS_MAX_TURNS = 60`。两者**尚未接 settings.json**(要改得改代码,见 [PLAN.md](PLAN.md) 未决) |
