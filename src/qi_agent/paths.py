...
QI_AGENT_CONFIG = "QI_AGENT_CONFIG"  # 指向 qi_agent.toml 具体文件(最高优先级)
...
def config_file_candidates(cwd: Path | None = None) -> list[Path]:
    """按优先级从高到低返回可能的 qi_agent.toml 路径(含 env 指定文件)。"""
    candidates: list[Path] = []
    env_file = os.environ.get(QI_AGENT_CONFIG)
    if env_file:
        candidates.append(Path(env_file).expanduser())
    candidates.append(project_home(cwd) / "qi_agent.toml")
    candidates.append(global_home() / "qi_agent.toml")
    return candidates
