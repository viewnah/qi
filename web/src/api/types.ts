...
export interface AguiMessage {
  id?: string;
  role: "user" | "assistant" | "system" | "tool" | "developer" | "reasoning" | "activity";
  content: string;
  name?: string;
}
...
export type AguiEvent =
  // 生命周期
  | (Base & { type: "RUN_STARTED"; threadId: string; runId: string; parentRunId?: string })
  | (Base & { type: "RUN_FINISHED"; outcome?: { type: string; interrupts?: unknown[] }; result?: string })
  ...
  // 工具
  | (Base & { type: "TOOL_CALL_START"; toolCallId: string; toolCallName: string; parentMessageId?: string })
  | (Base & { type: "TOOL_CALL_ARGS"; toolCallId: string; delta: string })
  | (Base & { type: "TOOL_CALL_END"; toolCallId: string })
  | (Base & { type: "TOOL_CALL_RESULT"; messageId: string; toolCallId: string; content: string; role?: string })
  // 状态
  ...
  | (Base & { type: "ACTIVITY_SNAPSHOT"; messageId: string; activityType: string; content: unknown; replace?: boolean })
  | (Base & { type: "ACTIVITY_DELTA"; messageId: string; activityType: string; patch: unknown[] })
  // 扩展点
